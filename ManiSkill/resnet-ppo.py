...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, obs):
        if obs["rgb"].shape[3] == 6:
           obs["rgb"] = obs["rgb"].float().permute(0,3,1,2)
        obs["rgb"] = obs["rgb"] / 255
        feats_1 = self.extractors(obs["rgb"][:, :3, :, :]) 
        feats_2 = self.extractors(obs["rgb"][:, 3:, :, :])
...
class Agent(nn.Module):
    def __init__(self, envs, checkpoint_path=None):
        super().__init__()
        self.feature_actor = FeatureExtractor()
        if checkpoint_path:
            checkpoint = torch.load(checkpoint_path, map_location="cuda")  
            self.feature_actor.load_state_dict(checkpoint["model_state_dict"])

            for param in self.feature_actor.parameters():
                param.requires_grad = False
            self.feature_actor.eval()
    ...
    def train_actor(self):
        for param in self.feature_actor.mlp.parameters():
            param.requires_grad = True
        self.feature_actor.mlp.train()
    def eval_actor(self):
        self.feature_actor.eval()
    def get_features(self, x):
        return self.feature_actor.get_features(x)
    def get_value(self, x):
        x = self.feature_actor.get_features(x)
        return self.critic(x)
    def get_action(self, x, deterministic=False):
        action_mean = self.feature_actor(x)
        if deterministic:
            return action_mean
    ...
    def get_action_and_value(self, x, action=None):
        feat = self.feature_actor.get_features(x)
        action_mean = self.feature_actor(x)
        action_logstd = self.actor_logstd.expand_as(action_mean)
        action_std = torch.exp(action_logstd)
        probs = Normal(action_mean, action_std)
...
if True:
    ...
    envs = TimeLimitWrapper(envs, max_episode_steps=80)
    ...
    agent = Agent(envs, checkpoint_path=args.checkpoint).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if next_obs["rgb"].shape[1] == 6:
                next_obs["rgb"] = next_obs["rgb"].permute(0, 2, 3, 1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if iteration > 15 and approx_kl < args.target_kl:
                    agent.train_actor()
                    evaluate = False
